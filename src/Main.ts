...
async function CreateAptitudeData(value: EntRaceStudyData[]){
    ...
    const HorseID = value.map(x => x.HorseID)
    ...
    Object.keys(dic).forEach(async (keyHorseID) => {
        p = p.then(async() => {
            ...
            Object.keys(entitys).forEach(keynum => {
                ...
                let str = `${represent.GoalTime},${represent.Venue},${represent.Range},${represent.Ground},${represent.GroundCondition},${represent.HoldMonth},${represent.Hold},${represent.Day},${represent.Weight},${represent.TrainerID},${represent.HorseGender},${represent.HorseWeight},${represent.Fluctuation},${represent.JockeyID},${represent.HorseAge}`
                str += blood
                value.forEach((row: EntRaceStudyData) => {
                    if (row.HorseID == HorseID) {
                        if (row.HoldDay.getTime() < entity.represent.HoldDay.getTime()){

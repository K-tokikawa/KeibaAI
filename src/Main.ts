...
import GetRaceStudyDataMinMax from "./querry/GetRaceStudyDataMinMax"
...
import ClassRaceData from "./class/ClassRaceData"
...
async function main(mode: number) {
    ...
    let sql: GetBloodStudyDataCount | GetJockyStudyDataCount | GetBloodStudyDataCount_Blood | GetRaceStudyDataMinMax
    switch (mode) {
        case 1:
            ...
            break
        case 2:
            sql = new GetJockyStudyDataCount()
            Count = await sql.Execsql() as EntStudyDataCount[]
            break
        case 3:
        ...
        case 5:
        case 6:
            sql = new GetRaceStudyDataMinMax()
    ...
    do {
        ...
        if (bolstart) {
            ...
            let rows: string[] = []
            let sql: GetBloodStudyData | GetJockyStudyData | GetBloodStudyData_Blood | GetRaceStudyData
            let value: EntBloodStudyData[] | EntJockyStudyData[] | EntBloodStudyData_Blood[] | EntRaceStudyData[]
            let filePath = ""
            switch (mode){
                case 1:
                ...
                case 2:
                    ...
                    console.log(`${count}`)
                    rows = value.map(x => { return `${x.Rank},${x.JockeyID},${x.HorseGender},${x.Venue},${x.Range},${x.Ground},${x.GroundCondition},${x.GateNo},${x.Age},${x.HoldMonth},${x.Weather},${x.Popularity},${x.Weight},${x.Hold},${x.Day},${x.Round}`})
                ...
                case 6:
                    ...
                    value = await sql.Execsql() as EntRaceStudyData[]
                    console.log(`${count}`)
                    rows = await CreateAptitudeData(value)
...
function CreateRotationData(value: EntRaceStudyData[]){
    const dic: {
        [horseID: number]:{[num: number]: ClassRaceData[]}
    } = { }
    value.forEach((row: EntRaceStudyData) => {
        const before = row.before == null ? 0 :(row.before.getTime() - row.HoldDay.getTime()) / 86400000
        const data = new ClassRaceData(
            row,
            before
        )
        const num = row.num
        ...
        Object.keys(dic[HorseID]).forEach(strnum =>{
            ...
            if (row.length > 1) {
                row.forEach((value: ClassRaceData) => {
                    if (data == ''){
                        data += `${value.GoalTime},${value.OutValue},${value.Direction},${value.Venue},${value.HoldMonth},${value.Hold},${value.Day},${value.Range},${value.Ground},${value.GroundCondition},${value.Weight},${value.TrainerID},${value.HorseGender},${value.HorseWeight},${value.Fluctuation},${value.JockeyID},${value.before}`
                    } else {
...
function CreateAchievementData(value: EntRaceStudyData[]){
    ...
    Object.keys(dic).forEach(keyHorseID => {
        ...
        Object.keys(entitys).forEach(key => {
            ...
            let str = `${value.GoalTime},${value.OutValue},${value.Venue},${value.Range},${value.Ground},${value.GroundCondition},${value.HoldMonth},${value.Hold},${value.Day},${value.Weight},${value.TrainerID},${value.HorseGender},${value.HorseWeight},${value.Fluctuation},${value.JockeyID}`
...
async function CreateAptitudeData(value: EntRaceStudyData[]){
    const rows: string[] = []
    const dic: {
        [HorseID: number]: {
    ...
    Object.keys(dic).forEach(async (keyHorseID) => {
            ...
            const x: EntBloodStudyData_Blood = valueblood[0]
            ...
            Object.keys(entitys).forEach(keynum => {
                ...
                let str = `${represent.GoalTime},${represent.OutValue},${represent.Venue},${represent.Range},${represent.Ground},${represent.GroundCondition},${represent.HoldMonth},${represent.Hold},${represent.Day},${represent.Weight},${represent.TrainerID},${represent.HorseGender},${represent.HorseWeight},${represent.Fluctuation},${represent.JockeyID},${represent.HorseAge}`
                str += blood
                value.forEach((row: EntRaceStudyData) => {
                    if (row.HorseID == HorseID) {
                        if (row.HoldDay.getTime() < entity.represent.HoldDay.getTime()){

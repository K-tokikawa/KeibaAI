...
function CreateRotationData(value: EntRaceStudyData[]){
    interface HorseRotation {
        [horseID: number]:{[num: number]: ClassRotationData[]}
    }
    const dic: HorseRotation = { }
    value.forEach((row: EntRaceStudyData) => {
        const before = row.before == null ? 0 :(row.before.getTime() - row.HoldDay.getTime()) / 86400000
        const data = new ClassRotationData(
            row.GoalTime,
            row.OutValue,
            row.Direction,
            row.Venue,
    ...
    Object.keys(dic).forEach(strkey => {
        ...
        Object.keys(dic[HorseID]).forEach(strnum =>{
            ...
            if (row.length > 1) {
                row.forEach((value: ClassRotationData) => {
                    if (data == ''){

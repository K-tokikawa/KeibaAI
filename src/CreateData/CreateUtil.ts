...
import ClassDicRace from "../class/ClassDicRace"
...
export async function CreateTotallingData(valuenum: number){
    const sqlCount = new GetHorseIDsCount()
    const Count = await sqlCount.Execsql() as EntStudyDataCount[]
    const Datanum = Count[0].Count
    console.log(Datanum)
    let registloop = GetLoop(Datanum, valuenum)
    let count = 0
    const ProgressBar = simpleProgress()
    const progress = ProgressBar(registloop, 200, 'Regist')

    while (registloop > 0) {
        const [Start, Finish] = GetStartFinish(valuenum, count, registloop, Datanum)
        let param = new PrmStudyData(Start, Finish)
        const sql = new GetHorseIDs(param)
        const value = await sql.Execsql() as EntHorseIDsData[]
        ...
        const deletesql = new DeletePredictRecord(paramHorseIDs)
...
export function GetDicRace(value: EntRaceInfomationData[]): [ClassDicRace, number[]]
{
    const dicRace: ClassDicRace = {}
...
export async function GetDicHorse(RaceIDs: number[]): Promise<[ClassDicHorse, number[]]>
{
    const dicHorse: ClassDicHorse = {}
...
export async function GetBloodData(HorseIDs: number[])
{
    const bloodparam = new PrmStudyData(0,0, HorseIDs)
    const blood = new GetHorseIDBloodStudyData_Blood(bloodparam)
    const bloodvalue = await blood.Execsql() as EntBloodStudyData[]

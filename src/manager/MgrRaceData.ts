...
export default class MgrRaceData{
    ...
    async dicCreate(RaceData: EntRaceHorseStudyData[]){
        const dic: {
            [HorseID: number]: {
                [num: number]: {PassageData: ClassPassageData, AchievementData: ClassAchievementData, RaceHorseData: ClassRaceHorseData[]}
            }
        } = {}
        const Passagerow: string[] = []
        const Achievementrow: string[] = []
        const Rotationrow: string[] = []
        RaceData.forEach((row: EntRaceHorseStudyData) => {
            ...
            if (dic[HorseID][num] == undefined){
                dic[HorseID][num].RaceHorseData = []
            }
            const PassageData = new ClassPassageData(data)
            dic[HorseID][num].PassageData = PassageData
            const AchievementData = new ClassAchievementData(data)
            dic[HorseID][num].AchievementData = AchievementData
            dic[HorseID][num].RaceHorseData.push(data)
            if (num - 1 > 0) {
                dic[HorseID][num - 1].RaceHorseData.push(data)
            }
            if (num - 2 > 0) {
                dic[HorseID][num - 2].RaceHorseData.push(data)
            }
            if (num - 3 > 0) {
                dic[HorseID][num - 3].RaceHorseData.push(data)
            }
            if (num - 4 > 0) {
                dic[HorseID][num - 4].RaceHorseData.push(data)
            }
            if (num - 5 > 0) {
                dic[HorseID][num - 5].RaceHorseData.push(data)
            }
        })
        ...
        Object.keys(dic).forEach(async (keyHorseID) => {
            p = p.then(async() => {
                ...
                const entitys: {[num: number]: {PassageData: ClassPassageData, AchievementData: ClassAchievementData, RaceHorseData: ClassRaceHorseData[]}} = dic[HorseID]
                Object.keys(entitys).forEach(keynum => {
                    ...
                    const RaceHorseData = dic[HorseID][num].RaceHorseData
                    let data = ''
                    if (RaceHorseData.length > 1) {
                        ...
                        Rotationrow.push(data)
                    }
                    const represent = Passageentity.represent
                    let strPassage = `${represent.GoalTime},${represent.Venue},${represent.Range},${represent.Weather}.${represent.Ground},${represent.GroundCondition},${represent.HoldMonth},${represent.Hold},${represent.HorseNo},${represent.Day},${represent.Weight},${represent.TrainerID},${represent.HorseGender},${represent.HorseWeight},${represent.Fluctuation},${represent.JockeyID},${represent.HorseAge}`
                    strPassage += blood
                    ...
                    Object.keys(entitys).forEach(key => {
                        ...
                        let strAchievement = `${value.GoalTime},${value.Venue},${value.Range},${value.Ground},${value.HorseAge},${value.GroundCondition},${value.HoldMonth},${value.Hold},${value.Day},${value.Weather},${value.Weight},${value.TrainerID},${value.HorseGender},${value.HorseWeight},${value.HorseNo},${value.Fluctuation},${value.JockeyID}`
                        ...
                        Achievementrow.push(strAchievement)
                    })
                })
            })
        })
